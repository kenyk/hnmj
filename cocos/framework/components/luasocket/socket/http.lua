...
local mime = cc.load("luasocket").mime
...
--异步http请求 post
function _M:asyncPostRequest(url,callback,param,failCallBack)
...
--异步http请求 get
function _M:asyncGetRequest(url,callback,param,failCallBack)
    local xhr = cc.XMLHttpRequest:new() -- 新建一个XMLHttpRequest对象
    xhr.responseType = cc.XMLHTTPREQUEST_RESPONSE_STRING -- 相应类型为字符串
    xhr:open("GET", url)-- post方式
    local function onReadyStateChange()
        local statusCode = xhr.status
        print("Http Status Code:"..statusCode) -- 状态吗为200时成功
        if statusCode == 200 then
